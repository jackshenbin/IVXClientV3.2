...
namespace BOCOM.DataModel
{
    public class Utils
    {
        ...
        public static string GetByteSizeInUnit(ulong size)
        {
            ...
            if (size >= Common.VOLUMESIZE_G)
            {
                sRet = string.Format("{0} GB", size >> 30);
            }
            else if (size >= DataModel.Common.VOLUMESIZE_M)
            {
                sRet = string.Format("{0} MB", size >> 20);
            }
            else if (size >= DataModel.Common.VOLUMESIZE_K)
            {
        ...
        internal static DateTime ConvertLinuxTime(uint linuxtime)
        {
        ...
        internal static UInt32 ConvertLinuxTime(DateTime dnettime)
        {
