...
namespace IVX.DataModel
{
    public class SearchPara : SearchParaBase, ICloneable
    {
        ...
        public bool IsSimilaritySearch
        {
            ...
            {
                bool bRet = false;
                if (SearchType == DataModel.SearchType.Normal || SearchType == DataModel.SearchType.VideoPlayNormal)
                {
        ...
        public SearchItem GetSearchItem(string cameraCode)
        {
            SearchItem searchItem = null;
            ...
            if (SearchItems != null)
            {
                foreach (SearchItem item in SearchItems)
                {
                    if (string.Compare(item.CameraCode, cameraCode, true) == 0)
                    {
                        searchItem = item;
                        break;
                    }
        ...
        public SearchItem GetSearchItemBySearchKeyHandle(uint searchKeyHandle)
        {
            SearchItem searchItem = null;
            ...
            if (SearchItems != null)
            {
                foreach (SearchItem item in SearchItems)
                {
                    if(item.SearchHandle == searchKeyHandle)
                    {
                        searchItem = item;
                        break;
                    }
        ...
        public object Clone()
        {
            ...
            searchParaNew.PageInfo = this.PageInfo;
