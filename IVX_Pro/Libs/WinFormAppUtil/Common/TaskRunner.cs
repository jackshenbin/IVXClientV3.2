...
namespace WinFormAppUtil
{
    ...
    public class TaskRunner<TIN, TOUT> : ITask
    {
        ...
        public void AddTask(TaskItem<TIN, TOUT> taskItem)
        {
            lock (m_TaskItems)
            {
                ...
                if (toInsert)
                {
                    ...
                    LogService.GetLog("AppUtil").DebugFormat("{0}: Add Task item: {1}, Total: {2}",
        ...
        public bool Stop()
        {
            LogService.GetLog("AppUtil").DebugFormat("Entering TaskRunner.Stop, {0} ...", this.m_Name);
            bool result = false;
            StopAndClear();
            m_IsRunning = false;
            m_MRETaskAdded.Set();
            ...
            else
            {
                ...
                {
                    LogService.GetLog("AppUtil").DebugFormat("TaskRunner.Stop, {0}, wait for taskloop complete, {1}", this.m_Name, i);
                    Thread.Sleep(500);
            ...
            LogService.GetLog("AppUtil").DebugFormat("TaskRunner.Stop, {0}, taskloop exit: {1}", this.m_Name, result);
            ...
            while (m_IsRunning)
            {
                lock (m_TaskItems)
                {
                    ...
                    if (m_TaskItems.Count > 0)
                    {
                        ...
                        LogService.GetLog("AppUtil").DebugFormat("{0}: Dequeued Task item: {1}, Total: {2}",
                        m_Name, taskItem.GetHashCode().ToString(), m_TaskItems.Count);
                    }
                }
                ...
                if (hasItem)
                {
                    ...
                    {
                        LogService.GetLog("AppUtil").Error("LoopRunTask error: ", ex);
