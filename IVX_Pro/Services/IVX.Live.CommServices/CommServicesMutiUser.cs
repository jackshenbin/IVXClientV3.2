...
namespace IVX.Live.CommServices {
	public class CommServicesMutiUser {
		...
		public CommServicesMutiUser(string url) {
			IsConnected = false;
			WebserviceUrl = url;
        ...
        public List<BlackListLib> GET_BLACK_LIST(ulong Context)
        {
			MyLog4Net.Container.Instance.Log.DebugWithDebugView(string.Format("CommServices GET_BLACK_LIST"));
			/*
			string message = CommProtocol.BuildProtocolBody(MessageCmd.VDM_MSG_GET_BLACK_LIST_DATA_REQ, "");
			string rsp = Send(message);
		...
		public bool ADD_BLACK_LIST(ulong Context ,BlackListLib blackListLib) {
			bool ret = true;
		...
		public bool DEl_BLACK_LIST(ulong Context ,UInt32 blackListHandel, out string desStr) {
			bool ret = true;
        ...
        public uint ADD_SVERVER(ulong Context, string ip, uint port, E_VDA_SERVER_TYPE type, string des)
        {
        ...
        public void DEL_SVERVER(ulong Context, uint serverid)
        {
        ...
        public void MDF_SVERVER(ulong Context, uint serverid, string ip, uint port, E_VDA_SERVER_TYPE type, string des)
        {

		}
        public ServerInfoV3_1 GET_SVERVER(ulong Context, uint serverid)
        {
		...
		public List<ServerInfoV3_1> GET_SVERVER_LIST(ulong Context ) {
        ...
        public uint ADD_NET_STORE(ulong Context, E_VDA_NET_STORE_DEV_PROTOCOL_TYPE type, string name, string ip, uint port, string username, string password)
        {
        ...
        public void DEL_NET_STORE(ulong Context, uint netstoreid)
        {
		...
		public List<VideoSupplierDeviceInfo> GET_NET_STORE_LIST(ulong Context ) {
		...
		public List<CameraInfoV3_1> GET_CAMERA_LIST(ulong Context ) {
			MyLog4Net.Container.Instance.Log.DebugWithDebugView(string.Format("CommServices GET_CAMERA_LIST "));
			List<VideoSupplierDeviceInfo> devlist = GET_NET_STORE_LIST( Context );
        ...
        public uint ADD_CAMERA(ulong Context, uint netstoreid, string channel, string name, string cameraId, double x, double y)
        {
        ...
        public Dictionary<uint, string> ADD_CAMERA_LIST(ulong Context, List<CameraInfoV3_1> cams)
        {
        ...
        public void DEL_CAMERA(ulong Context, uint id)
        {
        ...
        public Dictionary<uint, string> ADD_TASK(ulong Context, List<TaskInfoV3_1> tasklist)
        {
        ...
        public void DEL_TASK(ulong Context, List<uint> taskidList)
        {
        ...
        public void MDF_TASK(ulong Context, uint taskid, uint priority, DateTime startTime, DateTime endTime, uint status, string taskname)
        {
        ...
        public void MDF_TASK(ulong Context, uint taskid, uint priority, DateTime startTime, DateTime endTime, string taskname)
        {
			MDF_TASK( Context ,taskid, priority, startTime, endTime, 0, taskname);

		}
        public void PAUSE_REALTIME_TASK(ulong Context, uint taskid)
        {
			MDF_TASK( Context ,taskid, 0, new DateTime(), new DateTime(), 1, "");

		}
        public void RESUME_REALTIME_TASK(ulong Context, uint taskid)
        {
			MDF_TASK( Context ,taskid, 0, new DateTime(), new DateTime(), 2, "");

		}

        public TaskInfoV3_1 GET_TASK(ulong Context, uint taskid)
        {
            StringBuilder sb = new StringBuilder();
            sb.Append("<Request>");
            ...
            string message = CommProtocol.BuildProtocolBody(Context,MessageCmd.VDM_MSG_GET_TASK_REQ, sb.ToString());
            string rsp = Send(message);
            if (string.IsNullOrEmpty(rsp))
		...
		//}
		public List<TaskInfoV3_1> GET_TASK_LIST(ulong Context ) {
			if (thGetTask.ThreadState == System.Threading.ThreadState.Unstarted)
				thGetTask.Start( Context );

			return isTaskListInited ? m_taskList.ToList() : null;
		}
        public List<TaskInfoV3_1> TASK_PAGING(ulong Context, uint pageIndex, uint percount, out uint totalcount)
        {
			totalcount = 0;
        ...
        public List<TaskInfoV3_1> TASK_PAGING(ulong Context, uint pageIndex, uint percount, E_TASK_GET_SORT_TYPE order, List<uint> tasktype, List<uint> anatype, List<uint> taskstat, out uint totalcount)
        {
			if (!isTaskListInited) {
				totalcount = 0;
				return null;
        ...
        public List<TaskProgressInfoV3_1> GET_TASK_PROGRESS(ulong Context, List<uint> taskIdList)
        {
			var temp = m_taskList.ToList();
        ...
        public List<TaskProgressInfoV3_1> GET_TASK_PROGRESS_BASE(ulong Context, List<uint> taskIdList)
        {
        ...
        public bool TASK_REANALYSE(ulong Context, uint taskId, E_VIDEO_ANALYZE_TYPE analyseType, string analyseParam, uint splitTime = 0)
        {
        ...
        public bool ADD_TASK_ANALYSETYPE(ulong Context, uint taskId, E_VIDEO_ANALYZE_TYPE analyseType, string analyseParam, uint splitTime = 0)
        {
        ...
        public bool DEL_TASK_ANALYSETYPE(ulong Context, uint taskId, E_VIDEO_ANALYZE_TYPE analyseType)
        {
		...
		public List<UploadTaskInfoV3_1> GET_DOWN_LOAD_LIST(ulong Context ) {
        ...
        public uint ADD_TRANS_EVENT(ulong Context, uint taskId, uint templateId, E_TRANCE_STORE_TYPE storeType, string storeServerIP, uint storeServerPort, E_TRANS_PROTOCOL_TYPE transProtocol, E_VIDEO_ANALYZE_TYPE analyseType)
        {
        ...
        public void DEL_TRANS_EVENT(ulong Context, uint id)
        {
		...
		public List<TransEvnetInfo> GET_TRANS_EVENT_LIST(ulong Context ) {
        ...
        public List<TransEvnetInfo> GET_TASK_EVENT_LIST(ulong Context, uint taskid)
        {
        ...
        public TaskStroreInfoV3_1 GET_RESULT_STORE_LIST(ulong Context, string cameraCode, E_VIDEO_ANALYZE_TYPE anaType)
        {
        ...
        public List<TaskStroreInfoV3_1> GET_TRAFFIC_RESULT_STORE_LIST(ulong Context, string cameraCode, E_VIDEO_ANALYZE_TYPE anaType)
        {
			MyLog4Net.Container.Instance.Log.DebugWithDebugView(string.Format("CommServices GET_RESULT_STORE_LIST cameraCode:{0},anaType:{1}", cameraCode, anaType));
			StringBuilder sb = new StringBuilder();
			sb.Append("<Request><RequestList>");
        ...
        public List<TaskMSSInfoV3_1> QUERY_TASK_MSS(ulong Context, List<uint> taskIdList)
        {
        ...
        public List<TaskBriefMSSInfoV3_1> QUERY_BRIEF_TASK(ulong Context, List<uint> taskIdList)
        {
		...
		// 获取日志信息
        public List<LogSearchResultInfo> GET_LOG_LIST_DATA(ulong Context, LogSearchParam param)
        {
		...
		// 获取用户信息
		public List<UserInfo> GET_USER_LIST_DATA(ulong Context ) {
		...
		// 添加指定用户  0表示添加成功
        public UInt32 ADD_USER_INFO_REQ(ulong Context, UserInfo usrInfo)
        {
		...
		// 删除指定用户  0表示删除成功
        public UInt32 DEL_USER_INFO_REQ(ulong Context, UInt32 userHandle)
        {
		...
		// 修改指定用户  0表示修改成功
        public UInt32 MOD_USER_INFO_REQ(ulong Context, UserInfo usrInfo)
        {
		...
		private void tReflashTaskList(object Context) {
			while (!isExit) {
				...
				while (true) {
					try {
						var list = TASK_PAGING((ulong) Context ,pageindex, 1000, out totalcount);
						var progresslist = GET_TASK_PROGRESS_BASE((ulong) Context ,
							list.Where(it =>
				...
				if (newList.Exists(it => it.StatusList.Exists(xx => (int)xx.Status < (int)E_VDA_TASK_STATUS.E_TASK_STATUS_IMPORT_EXECUTING))) {
					foreach (IVX.DataModel.UploadTaskInfoV3_1 item in GET_DOWN_LOAD_LIST((ulong) Context )) {
