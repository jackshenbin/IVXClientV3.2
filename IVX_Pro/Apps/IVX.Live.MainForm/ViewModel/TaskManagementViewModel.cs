...
namespace IVX.Live.MainForm.ViewModel
{
    class TaskManagementViewModel
    {
        ...
        {
            Framework.Container.Instance.CommService.TaskDeleted += new Action<DataModel.TaskInfoV3_1>(CommService_TaskDeleted);
            Framework.Container.Instance.CommService.TaskAdded += new Action<DataModel.TaskInfoV3_1>(CommService_TaskAdded);
        }

        void CommService_TaskAdded(DataModel.TaskInfoV3_1 obj)
        {
            if (TaskAdded != null)
                TaskAdded(obj);
        }
        ...
        public SortedDictionary<string, TaskTimeLine> GetTaskTimeLine()
        {
            ...
            foreach (var item in Framework.Container.Instance.CommService.GET_TASK_LIST(true))
            {
                string type = CalcTimelineType(item.StartTime);
        ...
        {
            List<uint> idlist = Framework.Container.Instance.CommService.GET_UNFINISHED_TASKID_LIST();
        ...
        internal void DeleteTask(uint taskid)
        {
            Framework.Container.Instance.CommService.DEL_TASK(new List<uint>() { taskid});
        }

        internal DataModel.TaskInfoV3_1 GetTaskInfo(uint taskid)
        {
